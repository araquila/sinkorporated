...
# FUNCTION
def Weights_Class_I(W_empty_jet, W_empty_tbp, W_payload, W_crew, C_fe, S, S_wet, A_jet, A_tbp, e_jet, e_tbp, cj_loiter_jet, cj_cruise_jet, eff_loiter_tbp, eff_cruise_tbp, cp_loiter_tbp, cp_cruise_tbp, f_trapped_fuel,  V_cruise_jet, V_cruise_tbp,jet = False, tbp = False):
...
# THIS SECTION IS USED FOR JET AIRCRAFT; FOR TURBOPROP AIRCRAFT, SCROLL DOWN
    ...
    # Determine the maximum L/D for jets
        ...
        f_fuel_jet, f_reserve_jet, f_cruise_start_jet, f_cruise_end_jet = fuel_fraction(LD_cruise_jet = LD_cruise_jet, cj_cruise_jet = cj_cruise_jet, cj_loiter_jet = cj_loiter_jet, LD_loiter_jet = LD_loiter_jet, V_cruise_jet, V_cruise_tbp,jet = True)
...
# THIS SECTION IS USED FOR TURBOPROP AIRCRAFT
    ...
    # Determine the maximum L/D for a turboprop
        ...
        f_fuel_tbp, f_reserve_tbp,f_cruise_start_tbp, f_cruise_end_tbp = fuel_fraction(LD_loiter_tbp = LD_loiter_tbp, LD_cruise_tbp = LD_cruise_tbp, eff_cruise_tbp = eff_cruise_tbp, eff_loiter_tbp = eff_loiter_tbp, cp_cruise_tbp = cp_cruise_tbp, cp_loiter_tbp = cp_loiter_tbp, V_cruise_jet, V_cruise_tbp,tbp = True)
