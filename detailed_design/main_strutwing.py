...
emptylist = []
maxlist =[]
fuselagefactor = np.arange(-0.1,0.11,0.05)
...
for i in range(len(fuselagefactor)):    
    ...
    #    W_wing = det_wing_weight_new(b, S_tbp, sweepqc, taper, MTOM_tbp, V_cruise_tbp, t_c_ratio)
        ...
        W_fus = pounds_to_kg(det_fuselage_weight(kg_to_pounds(MTOM_tbp), 1.5*ult_load_factor(kg_to_pounds(MTOM_tbp)), meter_to_feet((fuselagefactor[i]+1)*length_fuselage), metersquared_to_feetsquared(np.pi*diameter_fuselage_outside*length_fuselage), taper, meter_to_feet(b), sweepqc, LD_cruise_tbp, fuselage_mounted_lg=True))
...
#    print(lateral_position)
    mtomlist.append(MTOM_tbp)
    oewlist.append(OEW_tbp/g)
    fuellist.append(fuel_per_passenger_tbp_1000)
#plt.scatter(n_passenger,mtomlist,label="MTOM")
#plt.scatter(n_passenger, oewlist, label="OEM")
#
...
ax1.plot(fuselagefactor, mtomlist, color='b')
ax1.plot(fuselagefactor, oewlist, color='b')
#ax1.plot(n_passenger, oewlist, color='b')
ax1.set_xlabel('Number of passengers [-]')
#ax1.set_xlim([-0.5, 1.5])
ax1.set_ylim([9000, 20000])
...
ax2.plot(fuselagefactor, fuellist, color='r')
ax2.set_ylabel('Fuel burn / passenger [kg]', color='r')
ax2.tick_params('y', colors='r')
#ax2.set_xlim([-0.5, 1.5])
ax2.set_ylim([25.4, 25.8])
